...
(function (global) {
  ...
  function renderHarness(state, doc) {
    ...
    var harness = (state && state.meta && state.meta.harness) || 'Pi';
    var role = (state && state.meta && state.meta.harnessRole) || 'Coding Agent Harness';

    nameEl.textContent = harness;
    if (roleEl) roleEl.textContent = role;

    if (iconWrap) {
      if (/pi/i.test(harness)) {
        iconWrap.innerHTML = '<svg width="15" height="15" viewBox="0 0 800 800" fill="currentColor"><path fill-rule="evenodd" d="M165.29 165.29 H517.36 V400 H400 V517.36 H282.65 V634.72 H165.29 Z M282.65 282.65 V400 H400 V282.65 Z"/><path d="M517.36 400 H634.72 V634.72 H517.36 Z"/></svg>';
      } else if (/claude/i.test(harness)) {
